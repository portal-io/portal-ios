...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *identifier = [bundleId stringByAppendingString:@".BackgroundSession"];
...
- (NSData*)checkResumDataWithKey:(NSURL*)url toDirectory:(NSURL*)directory withName:(NSString*)name
{
    ...
    if (resumeData) {
        ...
        //获取已下载的Data
        //文件路径
        NSString * curDir = [directory.absoluteString stringByReplacingOccurrencesOfString:name withString:@""];
        NSString *filePath = [curDir stringByAppendingString:[@"tmp_" stringByAppendingString:name]];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location
{
    ...
    //校验文件大小
    BOOL isCompltedFile = (fileSize>0) && (fileSize == download.fileSize);
    if (isCompltedFile) {
        ...
        download.error = [[NSError alloc] initWithDomain:NSOSStatusErrorDomain code:1000 userInfo:@{@"msg":@"fileSize not true"}];
        NSLog(@"download 下载完成的文件大小不正确");
    }
