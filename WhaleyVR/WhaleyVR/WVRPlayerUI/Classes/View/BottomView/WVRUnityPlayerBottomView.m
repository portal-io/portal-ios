...
@interface WVRUnityPlayerBottomView ()<WVRSliderDelegate> {
    id _rac_handler;
}
...
@implementation WVRUnityPlayerBottomView
...
- (UILabel *)durationLabel {
    if (!_durationLabel) {
        ...
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(label.width);
            make.height.mas_equalTo(label.height);
            make.right.equalTo(self.definitionBtn.mas_left).offset(0 - fitToWidth(VIEW_SPACE));
            make.centerY.equalTo(label.superview);
        }];
    }
...
- (void)definitionChanged {
    ...
    [self.definitionBtn setTitle:self.viewModel.definition forState:UIControlStateNormal];
}
