...
@interface WVRLiveRecBannerView ()<UIGestureRecognizerDelegate>
...
@implementation WVRLiveRecBannerView
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                WVRLiveRecBannerItemView *cell = [_cells objectAtIndex:i];
...
- (void)reloadData
{
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:NSStringFromClass([WVRLiveRecBannerItemView class])]) {
...
- (UIView *)dequeueReusableCell{
    WVRLiveRecBannerItemView *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//    NSLog(@"_scrollView.contentOffset.x %f",_scrollView.contentOffset.x);
    NSInteger pageIndex;
    CGFloat curX = _scrollView.contentOffset.x;
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:
            
            if (_scrollView.contentOffset.x < self.currentPageIndex*_pageSize.width) {
                curX += _pageSize.width/2;
            }
            pageIndex = (int)floor(curX / _pageSize.width) % self.orginPageCount;
            break;
        case NewPagedFlowViewOrientationVertical:
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if ([touch.view isKindOfClass:[UISlider class]]||[touch.view isKindOfClass:[GLKView class]]||[touch.view isKindOfClass:[WVRPlayerView class]]){
        return NO;
    }
