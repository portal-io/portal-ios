//
//  WVRSpring2018WebVC.m
//  WhaleyVR
//
//  Created by Bruce on 2018/1/29.
...
@interface WVRSpring2018WebVC ()

@end


@implementation WVRSpring2018WebVC

- (void)viewDidAppear:(BOOL)animated {
    [super viewDidAppear:animated];
    
    WVRNavigationController *nav = (WVRNavigationController *)self.navigationController;
    if ([nav respondsToSelector:@selector(setGestureInValid:)]) {
        [nav setGestureInValid:YES];
    }
}
...
- (void)drawUI {
    ...
    if (!self.isNews && [self.URLStr containsString:@"topBarTransparent=1"]) {
...
- (void)rightBarShareItemClick {
    ...
    WVRShareType type = self.isNews ? WVRShareTypeNews : WVRShareTypeH5;
    ...
    self.shareV = [WVRUMShareView shareWithContainerView:rootVC.view
                                                     sID:@""
                                                 iconUrl:nil
                                                   title:[WVRSpring2018Manager shareInstance].infoModel.displayName
                                                   intro:@""
...
- (NSString *)biPageName {
    ...
    return [WVRSpring2018Manager shareInstance].infoModel.displayName;
}
...
- (void)receiveWebShareDoneNotification:(NSNotification *)noti {
    ...
    [self.webView execSend:self.shareCallBackId withPayload:dict];
    
    self.shareCallBackId = @"";
}
...
- (void)exitPage {
    ...
    if ([self.backDelegate respondsToSelector:@selector(backForResult:resultCode:)]) {
        [self.backDelegate backForResult:nil resultCode:10];
    }
}
